...
A: Да, включён встроенный лимитер запросов на уровне Fastify (`@fastify/rate-limit`).

По умолчанию значения задаются через переменные окружения:

```
# Кол-во запросов на окно (на клиента, ключ определяется Authorization/x-api-key/IP)
RATE_LIMIT_MAX=5
# Длительность окна
# Примеры: '10 seconds', '1 minute', '1 hour'
RATE_LIMIT_WINDOW=10 seconds
```

Исключены из лимита: `/health` и `/api/docs`.
Рекомендуется также держать rate limiting на уровне reverse proxy/API Gateway как первую линию защиты.
