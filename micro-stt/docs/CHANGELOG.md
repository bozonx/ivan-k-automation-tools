...
- **Встроенный rate limiting** (19.10.2025)
  - Добавлен `@fastify/rate-limit` для защиты от зацикленных/агрессивных клиентов
  - Значения по умолчанию настраиваются переменными окружения:
    - `RATE_LIMIT_MAX` — количество запросов в окне
    - `RATE_LIMIT_WINDOW` — длительность окна (например, `10 seconds`, `1 minute`)
  - Ключ клиента определяется заголовком `Authorization` или `x-api-key`, иначе IP
  - Эндпоинты `/health` и `/api/docs` исключены из лимита
  - Обновлена документация: `docs/AUTH.md`, примеры в `env.*.example`
