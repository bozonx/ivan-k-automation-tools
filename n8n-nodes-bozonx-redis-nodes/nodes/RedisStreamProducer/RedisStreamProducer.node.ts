	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				options: [
					...
					},
					{
						...
						value: 'json',
						description:
							'Send a single field named "data" with the JSON string of the provided JSON or the incoming item (if left empty)',
					},
				],
				default: 'kv',
			...
			},
			{
				...
				default: '',
				description:
					'Content used by Text and JSON modes. For Text, sent as-is in field "payload". For JSON, provide valid JSON or leave empty to use the incoming item as JSON.',
			},
			{
				displayName: 'Payload',
				name: 'payload',
				...
				if (payloadMode === 'text') {
					...
					if (raw) {
						try {
							...
							throw new NodeOperationError(this.getNode(), 'Invalid JSON in Payload', {
								itemIndex: i,
							});
						}
					} else {
						...
						switch (p.type) {
							...
							case 'json': {
								try {
									...
									throw new NodeOperationError(this.getNode(), `Invalid JSON for key "${p.key}"`, {
										itemIndex: i,
									});
								}
								break;
